...
d3.csv("us-states.csv", d3.autoType).then(function (data) {
  ...
  let margin = ({ top: 0, right: 6, bottom: 6, left: 0 });

  let height = margin.top + barSize * (n * 2.5) + margin.bottom;

  let width = margin.left + barSize * 53 + margin.right;

  let x = d3.scaleLinear([0, 1], [margin.left, width - margin.right]);
  let x2 = d3.scaleLinear().domain([0,500000]).range([1,250]);

  let y = d3.scaleBand().domain(d3.range(n + 1)).rangeRound([margin.top, margin.top + barSize * (350 + 1 + 0.1)]).padding(0.1);
...
async function chart(keyframes, duration, height, width, prev, next, x, y, margin, barSize, x2)
{
  ...
  //yield svg.node();
  d3.select("#sorting").append(() => svg.node());
